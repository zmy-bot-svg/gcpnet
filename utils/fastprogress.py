...
class ProgressBar():
    ...
    def __init__(self, gen, total=None, display=True, leave=True, parent=None, master=None, comment=''):
        ...
        self.total = None if total == 'noinfer' else len(
            gen) if total is None else total
        self.last_v = 0
        if parent is None:
    ...
    def update_bar(self, val):
        ...
        remaining_t = '?' if self.pred_t is None else format_time(
            self.pred_t - elapsed_t)
        elapsed_t = format_time(elapsed_t)
        end = '' if len(self.comment) == 0 else f' {self.comment}'
        self.on_update(
            val, f'{pct}[{val}/{tot} {elapsed_t}{self.lt}{remaining_t}{end}]')
...
class NBMasterBar(MasterBar):
    ...
    def __init__(self, gen, total=None, hide_graph=False, order=None, clean_on_interrupt=False, total_time=False):
        ...
        ], clean_on_interrupt, total_time
        self.inner_dict = {'pb1': self.main_bar, 'text': ""}
        self.text, self.lines = "", []

    def on_iter_begin(self):
        self.html_code = '\n'.join(
            [html_progress_bar(0, self.main_bar.total, ""), ""])
        display(HTML(html_progress_bar_styles))
    ...
    def show(self):
        ...
        self.html_code = '\n'.join(
            [getattr(self.inner_dict[n], 'progress', self.inner_dict[n]) for n in to_show])
...
class ConsoleProgressBar(ProgressBar):
    ...
    def on_iter_end(self):
        if not self.leave and printing():
            print(f'\r{self.prefix}' + ' ' * (self.max_len -
                  len(f'\r{self.prefix}')), end='\r', flush=FLUSH)
        super().on_iter_end()
...
class ConsoleMasterBar(MasterBar):
    ...
    def write(self, line, table=False):
        def fstr(x): return f'{x:.3}' if isinstance(x, float) else str(x)
        if table:
            text = ''
            if not hasattr(self, 'names'):
                self.names = [
                    name + ' ' * (8-len(fstr(name))) if len(fstr(name)) < 8 else fstr(name) for name in line]
                text = '  '.join(self.names)
            else:
                for (t, name) in zip(line, self.names):
                    text += fstr(t) + ' ' * (2 + len(fstr(name))-len(fstr(t)))
