...
class WandbCallback:
    ...
    def on_validation_epoch_end(self, model: "KerasModel"):
        ...
        if n == 1:
            for m in dfhistory.columns:
                wandb.define_metric(name=m, step_metric='epoch',
                                    hidden=False if m != 'epoch' else True)
            wandb.define_metric(name='best_'+model.monitor,
                                step_metric='epoch')
    ...
    def on_fit_end(self, model: "KerasModel"):
        ...
        dfhistory = pd.DataFrame(model.history)
        dfhistory.to_csv(os.path.join(
            wandb.run.dir, 'dfhistory.csv'), index=None)
        # not save the model to save time
        # save ckpt
        # if self.save_ckpt:
        ...
        # plotly metrics
        metrics = [x.replace('train_', '').replace('val_', '')
                   for x in dfhistory.columns if 'train_' in x]
        ...
        # local save
        import shutil
        shutil.copy(model.ckpt_path, os.path.join(
            run_dir, os.path.basename(model.ckpt_path)))
